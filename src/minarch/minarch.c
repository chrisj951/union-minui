...
static int enable_sound_on_ff = 1;
...
static struct Config {
	...
	.frontend = (OptionList){
		...
		.options = (Option[]){
			...
			[FE_OPT_SOUND_WHEN_FF] = {
				.key	= "minarch_enable_sound_on_ff", 
				.name	= "Sound when FF",
				.desc	= "Enable or disable sound when fastforwarding",
				.default_value = 1,
				.value = 1,
				.count = 2,
				.values = onoff_labels,
...
static void Config_syncFrontend(int i, int value) {
	switch (i) {
		...
		case FE_OPT_SOUND_WHEN_FF: enable_sound_on_ff = value; break;
